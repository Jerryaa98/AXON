...
from model.step_profiler import StepProfiler
import types
...
class Dream(LM):
    def __init__(
        ...
        profile_steps: Optional[bool] = False,
        profile_tag: Optional[str] = "",
        ...
        **kwargs,
        ...
        self.axon_reveal_policy = str(axon_reveal_policy)
        self.axon_step_combo_mode = str(axon_step_combo_mode)
        self.axon_need_gate_mode = str(axon_need_gate_mode)
        self.axon_need_min_reveal_frac = float(axon_need_min_reveal_frac)
        self.axon_need_residual_conf_threshold = float(axon_need_residual_conf_threshold)
        self.axon_need_graph_coverage_threshold = float(axon_need_graph_coverage_threshold)
        self.axon_plugin = str(axon_plugin)
        self.axon_cover_target = float(axon_cover_target)
        self.axon_max_reveal_frac = float(axon_max_reveal_frac)
        self.axon_needed_rule = str(axon_needed_rule)
        self.axon_need_block_conf_threshold = float(axon_need_block_conf_threshold)
        self.axon_need_high_conf_frac_threshold = float(axon_need_high_conf_frac_threshold)
        self.axon_need_min_step = int(axon_need_min_step)
        self.axon_need_bad_streak = int(axon_need_bad_streak)
        self.axon_dawn_tau_sink = float(axon_dawn_tau_sink)
        self.axon_dawn_tau_edge = float(axon_dawn_tau_edge)
        self.axon_dawn_tau_induce = float(axon_dawn_tau_induce)
        self.axon_dawn_conf_threshold = float(axon_dawn_conf_threshold)
        self.axon_max_anchors_per_step = int(axon_max_anchors_per_step)
        self.axon_anchor_size_penalty = float(axon_anchor_size_penalty)
        self.axon_anchor_size_penalty_power = float(axon_anchor_size_penalty_power)
        ...
        self.axon_step_mode = str(axon_step_mode)
        self.unlock_stage2_fill = bool(unlock_stage2_fill) if not isinstance(unlock_stage2_fill, str) else unlock_stage2_fill.strip().lower() in {"true", "1", "yes"}
        ...
        self.axon_submodular_calls = 0
        self.axon_submodular_anchors = 0
        self.axon_step_records = []
    @property
    def batch_size(self):
    ...
    def _generate_batch(self, prompts: List[str], force_chat: bool = False) -> List[str]:
        # force_chat: set for 0-shot HumanEval (see generate_until) -- Dream-Instruct is an
        # instruct model and cannot complete a bare code stub in raw mode.
        if self.if_apply_chat_template or force_chat:
        ...
        generation_ids, nfe = self.model.diffusion_generate(
            ...
            axon_reveal_policy=self.axon_reveal_policy,
            axon_step_combo_mode=self.axon_step_combo_mode,
            axon_need_gate_mode=self.axon_need_gate_mode,
            axon_need_min_reveal_frac=self.axon_need_min_reveal_frac,
            axon_need_residual_conf_threshold=self.axon_need_residual_conf_threshold,
            axon_need_graph_coverage_threshold=self.axon_need_graph_coverage_threshold,
            axon_plugin=self.axon_plugin,
            axon_cover_target=self.axon_cover_target,
            axon_max_reveal_frac=self.axon_max_reveal_frac,
            axon_needed_rule=self.axon_needed_rule,
            axon_need_block_conf_threshold=self.axon_need_block_conf_threshold,
            axon_need_high_conf_frac_threshold=self.axon_need_high_conf_frac_threshold,
            axon_need_min_step=self.axon_need_min_step,
            axon_need_bad_streak=self.axon_need_bad_streak,
            axon_dawn_tau_sink=self.axon_dawn_tau_sink,
            axon_dawn_tau_edge=self.axon_dawn_tau_edge,
            axon_dawn_tau_induce=self.axon_dawn_tau_induce,
            axon_dawn_conf_threshold=self.axon_dawn_conf_threshold,
            axon_max_anchors_per_step=self.axon_max_anchors_per_step,
            axon_anchor_size_penalty=self.axon_anchor_size_penalty,
            axon_anchor_size_penalty_power=self.axon_anchor_size_penalty_power,
            ...
            axon_step_mode=self.axon_step_mode,
        ...
        self.axon_submodular_calls += int(getattr(self.model, "_last_axon_submodular_calls", 0))
        self.axon_submodular_anchors += int(getattr(self.model, "_last_axon_submodular_anchors", 0))
        self.axon_step_records.extend(getattr(self.model, "_last_axon_step_records", []))
    ...
    def generate_until(self, requests: List[Instance], disable_tqdm: bool = False):
        ...
        # generation_utils._sample_block reads this off the model instance.
        self.model._step_profiler = prof if self.profile_steps else None
        # Mirror llada/eval_llada.py: reset the CUDA peak counters immediately before the
        # timed region so 'Peak GPU Memory *' below measures decoding, not model load.
        if torch.cuda.is_available():
        ...
        for batch_idx in range(0, len(requests), self.batch_size):
            ...
            # Dream-Instruct is an instruct model. Measured on 0-shot HumanEval (LIMIT=20):
            # raw-completion 15%, chat-templated 40%, expected ~55%. Chat-templating is the
            # main fix; the residual gap is that the chat reply is prose + a markdown
            # ```python fence, which poisons lm-eval's prompt+completion scoring. So for
            # HumanEval only: chat-template the prompt, SKIP the completion-style `until`
            # split, and extract the first fenced code block. Few-shot tasks (gsm8k 74%,
            # mbpp 56%) are healthy in raw mode and keep the existing path.
            # Mirrors llada/eval_llada.py:662 (Branch A) and sdar/eval_sdar.py.
            doc = getattr(batch_requests[0], "doc", None)
            task_id = str(doc.get("task_id", "")) if isinstance(doc, dict) else ""
            is_humaneval = task_id.lower().startswith("humaneval")

            responses, nfe = self._generate_batch(contexts, force_chat=is_humaneval)
            if self.profile_steps:
                prof.finish_problem(batch_idx, batch_size=len(contexts))
            total_nfe += nfe
            if is_humaneval:
                for i, r in enumerate(responses):
                    fences = re.findall(r"```[^\n]*\n(.*?)```", r, re.DOTALL)
                    responses[i] = fences[0] if fences else r
            elif not self.escape_until:
                for i, r in enumerate(responses):
        ...
        if self.show_speed:
            ...
            if self.outp_path is not None:
                ...
                _metrics = {
                    ...
                    'AXON Submodular Calls': self.axon_submodular_calls,
                    'AXON Submodular Anchors': self.axon_submodular_anchors,
                }
                if torch.cuda.is_available():
                    _metrics.update({
                        'Peak GPU Memory Allocated (GB)': torch.cuda.max_memory_allocated(self.device) / 1e9,
                        'Peak GPU Memory Reserved (GB)': torch.cuda.max_memory_reserved(self.device) / 1e9,
                    })
                if self.profile_steps:
                    _metrics.update(prof.summary())
                ...
                if self.profile_steps:
                    ...
                    _decoder = str(self.axon_base_proposer) if self.alg == "axon" else str(self.alg)
                    prof.write_csv(
                        os.path.join(dirpath if dirpath else '.', "step_profile.csv"),
                        family="dream",
                        model=(_tag[0] if len(_tag) > 0 else "dream"),
                        task=(_tag[1] if len(_tag) > 1 else ""),
                        arm=(_tag[2] if len(_tag) > 2 else ""),
                        decoder=_decoder,
                    )
                if self.axon_step_records:
                    step_metrics_path = os.path.join(dirpath if dirpath else '.', 'stag_step_metrics.csv')
                    fieldnames = [
                        'step_idx',
                        'block_idx',
                        'step_in_block',
                    ...
                    with open(step_metrics_path, 'w', encoding='utf-8', newline='') as sf:
                        ...
                        writer.writeheader()
                        for rec in self.axon_step_records:
