...
# === AXON conflict-matrix dump (env-gated; off by default) ====================
# When AXON_DUMP_ATTN=<dir> is set, dump the conflict + raw-attention-overlap
# matrices on the FIRST AXON fire of the first rank's first batch element.
# Zero impact when the env var is unset.
_AXON_DUMP_DIR = os.environ.get("AXON_DUMP_ATTN") or None
_AXON_CONFLICT_DUMPED = {"done": False}


def _axon_dump_conflict(a_candidate, conflict):
    if _AXON_DUMP_DIR is None:
        return
    if int(os.environ.get("LOCAL_RANK", "0")) != 0:
        return
    if _AXON_CONFLICT_DUMPED["done"]:
        return
    _AXON_CONFLICT_DUMPED["done"] = True
    os.makedirs(_AXON_DUMP_DIR, exist_ok=True)
    raw = 0.5 * (a_candidate + a_candidate.transpose(0, 1))
    raw_np = raw.detach().cpu().float().numpy()
    np.fill_diagonal(raw_np, 0.0)
    np.savez_compressed(
        os.path.join(_AXON_DUMP_DIR, "conflict_matrices.npz"),
        attn_overlap=raw_np,
        conflict=conflict.detach().cpu().float().numpy(),
    )
...
def select_axon_adaptive_gate_anchors(
    ...
    """Select STAG/AXON anchors with an adaptive reveal budget.
    ...
    for b in range(mask_index.shape[0]):
        ...
        if int(axon_fixed_k) > 0:
            # explicit per-fire anchor-budget override (k ablation)
            max_select = int(axon_fixed_k)
        ...
        if b == 0:
            _axon_dump_conflict(a_candidate, conflict)
