...
# Vendored + patched for the AXON/AXON port: flash_attn is unavailable (no CUDA toolkit) and
# unnecessary since we run with attn_implementation="eager" to expose attention maps.
try:
...
class SDARRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        # pure-torch RMSNorm (flash triton kernel unavailable); numerically equivalent
...
class SDARAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Patched for the AXON/AXON port: SDPA everywhere (flash-attn unavailable).
        # torch.all(mask) == full visibility -> no mask; else pass the (block-diffusion) mask.
        _mask = None if (attention_mask is not None and torch.all(attention_mask)) else attention_mask
